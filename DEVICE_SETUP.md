...
**ב-Windows, בלחיצה כפולה:** `build-apps.bat` ב-root של הריפו פותח חלון עם כל האפליקציות (אותו גילוי מהדיסק כמו `build-all.sh`). מסמנים את מה ש**לא** לבנות, וכל השאר נבנות אחת אחרי השנייה; תיבת "גם להתקין על המכשיר" מריצה `installDebug` במקום `assembleDebug`, ואם אין מכשיר מחובר ב-adb הסקריפט שואל אם להמשיך בבנייה בלבד. הבחירה נשמרת לפעם הבאה ב-`.build-apps-selection.json`, והפלט המלא של הריצה האחרונה ב-`build-apps.log` (שניהם מקומיים וב-`.gitignore`). בסוף מופיעה הודעת סיכום; אם משהו נכשל, חלון הקונסול נשאר פתוח עם השגיאה.
