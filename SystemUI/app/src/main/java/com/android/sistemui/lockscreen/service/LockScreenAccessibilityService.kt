package com.android.sistemui.lockscreen.service
...
import com.android.sistemui.lockscreen.ui.LockScreenScreen
...
class LockScreenAccessibilityService : AccessibilityService(), LifecycleOwner, SavedStateRegistryOwner, ViewModelStoreOwner {
    ...
    // ראו SystemUIActions.ACTION_CALL_RINGING לפירוט. כל עוד שיחה מצלצלת, מסך הנעילה
            ...
            when (intent?.action) {
                ...
                SystemUIActions.ACTION_CALL_RINGING -> {
                    ...
                    if (SystemUIState.foregroundPackage == HOME_PACKAGE) {
                        val launchIntent = Intent(SystemUIActions.ACTION_LAUNCH_CALL_UI)
                        launchIntent.setPackage("com.future.dialer")
                        sendBroadcast(launchIntent)
                    }
                }
                SystemUIActions.ACTION_CALL_ENDED -> {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        if (event?.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            event.packageName?.toString()?.let { com.android.sistemui.utils.SystemUIState.foregroundPackage = it }
        }
    }
    ...
    // הוא מה שמאפשר לקבל כאן אירועי מקש מכל אפליקציה, לא רק מ-SystemUI עצמה.
    ...
    override fun onKeyEvent(event: KeyEvent): Boolean {
        ...
        if (suppressForActiveCall && event.action == KeyEvent.ACTION_DOWN && event.repeatCount == 0) {
            when (event.keyCode) {
                KeyEvent.KEYCODE_CALL -> sendBroadcast(Intent(SystemUIActions.ACTION_ANSWER_CALL).setPackage("com.future.dialer"))
                KeyEvent.KEYCODE_ENDCALL -> sendBroadcast(Intent(SystemUIActions.ACTION_REJECT_CALL).setPackage("com.future.dialer"))
            }
        }
    ...
    private fun showLockScreen() {
        ...
        mainHandler.post {
            try {
                ...
                isVisible = true
                com.android.sistemui.utils.SystemUIState.isLockScreenVisible = true
    ...
    private fun hideLockScreen() {
        ...
        mainHandler.post {
            try {
                ...
                composeView = null
                isVisible = false
                com.android.sistemui.utils.SystemUIState.isLockScreenVisible = false
            } catch (e: Exception) {
